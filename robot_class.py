...
class robot:
    ...
    def sense(self):
        ...
        for idx in range(self.num_landmarks):
            x_new = self.landmarks[idx][0] - self.x +self.measurement_noise * self.rand()
            y_new = self.landmarks[idx][1] - self.y +self.measurement_noise * self.rand()
            
            if (abs(x_new) < self.measurement_range and abs(y_new) < self.measurement_range):
                measurements.append([idx,x_new,y_new])
